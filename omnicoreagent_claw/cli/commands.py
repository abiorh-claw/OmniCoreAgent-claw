"""CLI commands for omnicoreagent-claw."""
...
from omnicoreagent_claw import __version__, __logo__
from omnicoreagent_claw.config.schema import Config

app = typer.Typer(
    name="omnicoreagent-claw",
    help=f"{__logo__} omnicoreagent-claw — Personal AI Agent powered by OmniCoreAgent",
...
def _init_prompt_session() -> None:
    ...
    history_file = Path.home() / ".omnicoreagent-claw" / "history" / "cli_history"
...
def _print_agent_response(response: str, render_markdown: bool) -> None:
    ...
    console.print()
    console.print(f"[cyan]{__logo__} omnicoreagent-claw[/cyan]")
    console.print(body)
    console.print()
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} omnicoreagent-claw v{__version__}")
        raise typer.Exit()
...
def main(
    ...
    """omnicoreagent-claw — Personal AI Agent powered by OmniCoreAgent."""
    pass
...
@app.command()
def onboard():
    """Initialize omnicoreagent-claw configuration and workspace."""
    from omnicoreagent_claw.config.loader import get_config_path, load_config, save_config
    from omnicoreagent_claw.config.schema import Config
    from omnicoreagent_claw.utils.helpers import get_workspace_path
    ...
    console.print(f"\n{__logo__} omnicoreagent-claw is ready!")
    console.print("\nNext steps:")
    console.print("  1. Add your API key to [cyan]~/.omnicoreagent-claw/config.json[/cyan]")
    console.print("     Get one at: https://openrouter.ai/keys")
    console.print("  2. Chat: [cyan]oca-claw agent -m \"Hello!\"[/cyan]")
...
You are a powerful personal AI agent. You are proactive, concise, and capable.
...
- Learn from user preferences and act proactively when appropriate
- Manage the user's inbox, calendar, files, and daily tasks
""",
        "SOUL.md": """# Soul

I am Claw, a personal AI agent powered by OmniCoreAgent.
...
def _make_provider(config: Config):
    ...
    from omnicoreagent_claw.providers.custom_provider import CustomProvider
    ...
    from omnicoreagent_claw.providers.registry import find_by_name
    spec = find_by_name(provider_name)
    if not model.startswith("bedrock/") and not (p and p.api_key) and not (spec and spec.is_oauth):
        console.print("[red]Error: No API key configured.[/red]")
        console.print("Set one in ~/.omnicoreagent-claw/config.json under providers section")
        console.print("Run [cyan]oca-claw onboard[/cyan] to get started")
        raise typer.Exit(1)
...
def gateway_run(
    ...
    from omnicoreagent_claw.cron.types import CronJob
    from omnicoreagent_claw.heartbeat.service import HeartbeatService
    
    if verbose:
        import logging
        logging.basicConfig(level=logging.DEBUG)
    
    console.print(f"{__logo__} Starting omnicoreagent-claw gateway on port {port}...")
    ...
    async def on_cron_job(job: CronJob) -> str | None:
        ...
        if job.payload.deliver and job.payload.to:
            from omnicoreagent_claw.bus.events import OutboundMessage
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show omnicoreagent-claw runtime logs during chat"),
):
    ...
    if logs:
        logger.enable("omnicoreagent_claw")  
    else:
        logger.disable("omnicoreagent_claw")  
    ...
    def _thinking_ctx():
        ...
        return console.status("[dim]omnicoreagent-claw is thinking...[/dim]", spinner="dots")
...
def channels_status():
    """Show channel status."""
    from omnicoreagent_claw.config.loader import load_config
...
def _get_bridge_dir() -> Path:
    ...
    user_bridge = Path.home() / ".omnicoreagent-claw" / "bridge"
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # omnicoreagent_claw/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall omnicoreagent-claw")
        raise typer.Exit(1)
...
def cron_list(
    ...
    """List scheduled jobs."""
    from omnicoreagent_claw.config.loader import get_data_dir
    from omnicoreagent_claw.cron.service import CronService
...
def cron_add(
    ...
    from omnicoreagent_claw.config.loader import get_data_dir
    from omnicoreagent_claw.cron.service import CronService
    from omnicoreagent_claw.cron.types import CronSchedule
...
def status():
    ...
    console.print(f"{__logo__} OmniCoreAgent-Claw Status\n")

    console.print(f"Config: {config_path} {'[green]✓[/green]' if config_path.exists() else '[red]✗[/red]'}")
    console.print(f"Workspace: {workspace} {'[green]✓[/green]' if workspace.exists() else '[red]✗[/red]'}")

    if config_path.exists():
        from omnicoreagent_claw.providers.registry import PROVIDERS
